...
namespace CenterBackend.Controllers
{
    ...
    public class FileController : ControllerBase
    {
        ...
        public IActionResult DownloadZipFileBig()
        {
            try
            {
                string sourceFolder = Path.Combine(_webHostEnv.WebRootPath, "Backup/FileBackup");//方式A：压缩【wwwroot内的指定文件夹】(推荐，你的项目内文件，如之前的Upload文件夹)
                ...
                var fileBytes = System.IO.File.ReadAllBytes(tempZipPath);// 核心：读取压缩包，返回给浏览器【自动下载】
